...
pub static APPENDS: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!("aethel_safekeeper_appends_total", "WAL append requests received")
        .unwrap()
});

/// WAL bytes durably appended.
pub static WAL_BYTES: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!("aethel_safekeeper_wal_bytes_total", "WAL bytes durably appended")
        .unwrap()
});
...
pub static REPLICATED: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "aethel_safekeeper_replicated_total",
        "Replication appends received from a leader"
    )
    .unwrap()
});
...
pub static VOTES_GRANTED: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!("aethel_safekeeper_votes_granted_total", "Leadership votes granted")
        .unwrap()
});

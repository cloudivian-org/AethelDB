...
/// Serve the compute-control API on `listener`.
pub async fn serve_control(proxy: Arc<Proxy>, listener: TcpListener) -> anyhow::Result<()> {
    loop {
        ...
        tokio::spawn(async move {
            if let Err(err) = handle(proxy, socket).await {
...
async fn handle(proxy: Arc<Proxy>, mut socket: TcpStream) -> anyhow::Result<()> {
    ...
    let (status, body) = route(&proxy, method, path).await;
...
async fn route(proxy: &Arc<Proxy>, method: &str, path: &str) -> (u16, String) {
